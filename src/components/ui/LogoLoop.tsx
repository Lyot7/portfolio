    ...
    <div className={`relative overflow-hidden ${className}`}>
      ...
      >
        {duplicatedLogos.map((logo, index) => (
          <div
            key={`${logo.name}-${index}`}
            className="flex items-center gap-2 w-auto min-w-[100px] h-12 px-3 py-2 bg-white/5 border border-white/10 rounded-lg backdrop-blur-sm opacity-60 hover:opacity-100 hover:bg-white/10 hover:border-white/20 transition-all duration-300"
          >
            <img
              src={logo.src}
              alt={logo.alt}
              className="h-6 w-6 object-contain filter grayscale hover:grayscale-0 transition-all duration-300"
            />
